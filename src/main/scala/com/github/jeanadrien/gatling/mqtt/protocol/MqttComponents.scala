...
/**
  *
  */
case class MqttComponents(mqttProtocol: MqttProtocol, system : ActorSystem) extends ProtocolComponents with StrictLogging {
    ...
    def mqttEngine(session : Session, connectionSettings : ConnectionSettings) : Validation[MQTT] = {
        ...
        Success(new MQTT()).flatMap(mqttProtocol.configureMqtt(session)).flatMap(connectionSettings.configureMqtt(session))
    }
