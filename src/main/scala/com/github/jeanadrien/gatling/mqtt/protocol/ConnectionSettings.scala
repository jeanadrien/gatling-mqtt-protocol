...
case class ConnectionSettings(
    ...
    private[protocol] def configureMqtt(session: Session)(mqtt1: MQTT) : Validation[MQTT] = {
        ...
        Success(mqtt1).flatMap { mqtt =>
            clientId.map { _(session).map { cid => mqtt.setClientId(cid); mqtt }} getOrElse (Success(mqtt))
        } flatMap { mqtt =>
            userName.map { _(session).map { un => mqtt.setUserName(un); mqtt }} getOrElse (Success(mqtt))
        } flatMap { mqtt =>
            password.map { _(session).map { pswd => mqtt.setPassword(pswd); mqtt }} getOrElse(Success(mqtt))
        } flatMap { mqtt =>
            willTopic.map { _(session).map { wto => mqtt.setWillTopic(wto); mqtt }} getOrElse(Success(mqtt))
        } flatMap { mqtt =>
            willMessage.map { _(session).map { wme => mqtt.setWillMessage(wme); mqtt }} getOrElse(Success(mqtt))
        }
    }
